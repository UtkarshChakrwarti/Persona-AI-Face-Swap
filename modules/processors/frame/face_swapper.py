...
def draw_mouth_mask_visualization(
    ...
    # Bounding box drawing disabled (removed black border artifact)
...
def create_face_mask(face: Face, frame: Frame) -> np.ndarray:
    ...
    try: # Wrap main logic in try-except
        ...
        # Erode mask inward to eliminate black border artifact at face edges
        erode_k = 15
        erode_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (erode_k, erode_k))
        mask = cv2.erode(mask, erode_kernel, iterations=1)
        # Strong feather blur so edges blend invisibly into background
        blur_k_size = getattr(modules.globals, "face_mask_blur", 51) # Increased default
        blur_k_size = max(1, blur_k_size // 2 * 2 + 1) # Ensure odd and positive
        mask = gpu_gaussian_blur(mask, (blur_k_size, blur_k_size), 0)
        # Extra smoothing pass for a soft gradient edge
