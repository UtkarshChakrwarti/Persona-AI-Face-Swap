...
def create_face_mask(face: Face, frame: Frame) -> np.ndarray:
    ...
    if landmarks is not None:
        ...
        # Erode inward to remove hard black border, then feather edges
        erode_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (13, 13))
        mask = cv2.erode(mask, erode_kernel, iterations=1)
        # Strong feather so edges blend naturally
        mask = gpu_gaussian_blur(mask, (51, 51), 0)
...
def draw_mask_visualization(
    ...
    if draw_method == "ellipse" and len(polygon) > 50:  # For eyes
        ...
        try:
            ...
            pass  # rectangle fallback disabled
